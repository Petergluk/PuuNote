...
**API Ключи (Осознанное решение).**
Приложение использует гибридный подход для ключей Gemini: сначала проверяются пользовательские ключи в localStorage (которые он ввел в браузере), а если их нет (или они не работают), используются ключи из переменных окружения `.env` (вшитые сервером). В случае публичного развертывания ключи сервера попадают в бандл, что воспринимается системами аудита как "утечка". Это **осознанное решение** (по дизайну проекта), так как для публичного демо (MVP) используются аккаунты с бесплатным API, утечка которых не критична (при исчерпании лимитов просто будет ошибка). 
