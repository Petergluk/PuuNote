...
- **Продвинутый Markdown и Visual Editor:** Полная поддержка GitHub Flavored Markdown внутри карточек. Также встроен визуальный WYSIWYG редактор (на базе Tiptap) с floating toolbar. Важно: мы поддерживаем оба режима, не удаляйте ни один из них при рефакторингах! Риск XSS снижается за счет `rehype-sanitize` и ограничений протоколов ссылок/изображений.
...
- **Markdown и WYSIWYG**: react-markdown + remark-gfm + rehype-sanitize, а также @tiptap/react для визуального режима (WysiwygEditor).
