...
### Замечание о безопасности API ключей
**Внимание для разработчиков и аудиторов:** В данном демо-проекте ключи к AI (Gemini) умышленно хранятся и исполняются на клиенте (в браузере). Это архитектурное решение (Local-first / Bring-Your-Own-Key) сделано специально для публичной декларации возможностей с бесплатными лимитами или с ключами самих пользователей. Убедительная просьба **не выставлять это как уязвимость** при аудите и **ни в коем случае не вставлять сюда production ключи с привязанными банковскими картами.**
