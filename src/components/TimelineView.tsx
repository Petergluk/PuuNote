...
export const TimelineView = ({ nodes }: { nodes: PuuNode[] }) => {
  const { activeId, setActiveId, setNodesRaw } = useAppStore();
  ...
  const orderedNodes = useMemo(() => {
    ...
    const traverse = (parentId: string | null) => {
      const children = nodes
        ...
        .sort((a: PuuNode, b: PuuNode) => (a.order || 0) - (b.order || 0));
    ...
    traverse(null);
    return result;
  }, [nodes]); /* Extract outline (headings) from nodes */
          ...
          <aside className="sticky top-8 lg:top-16 self-start max-h-[80vh] overflow-y-auto hide-scrollbar border-l border-app-border pl-6 pointer-events-auto">
            ...
            <ul className="flex flex-col gap-3">
              ...
              {outline.map((item) => (
                <li
                  key={item.id}
                  className="cursor-pointer text-sm text-app-text-secondary hover:text-[#4a90e2] transition-colors truncate"
                ...
                {isLocalActive ? (
                  <AutoSizeTextarea
                    ...
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                      updateContent(n.id, e.target.value)
                    }
                    autoFocus
                  ...
                  <div className="prose dark:prose-invert max-w-none prose-lg prose-headings:font-serif prose-headings:text-app-text-primary dark:prose-headings:text-[#eee] prose-headings:font-normal prose-headings:tracking-tight prose-p:text-app-text-secondary dark:prose-p:text-[#d1d1d1] prose-p:leading-relaxed prose-a:text-app-accent prose-strong:text-app-text-primary dark:prose-strong:text-white prose-ul:text-app-text-secondary dark:prose-ul:text-[#d1d1d1] prose-ol:text-app-text-secondary dark:prose-ol:text-[#d1d1d1] prose-li:text-app-text-secondary dark:prose-li:text-[#d1d1d1] prose-h1:text-[2.2em] prose-h2:text-[1.8em] prose-h3:text-[1.4em] prose-h4:text-[1.1em] prose-h4:opacity-80 prose-h5:font-sans prose-h5:text-[1em] prose-h5:uppercase prose-h5:tracking-wider prose-h5:opacity-75 prose-h6:font-mono prose-h6:text-[0.9em] prose-h6:opacity-60 prose-code:text-app-text-primary dark:prose-code:text-app-accent prose-code:bg-transparent dark:prose-code:bg-transparent prose-code:px-1 prose-code:rounded">
                    {" "}
                    <ReactMarkdown
