     ...
     for (const line of lines) {
         if (line.trim().length === 0) {
             ...
             for (let c = 0; c < line.length; c++) {
                 ...
                 if (line[c] === ' ') { spaceCount += 1; charsToStrip += 1; }
                 else if (line[c] === '\t') { spaceCount += 4; charsToStrip += 1; }
                 else break;
             }
...
const parseMindMapFormat = (mdText: string): PuuNode[] => {
   ...
   for (let i = 0; i < lines.length; i++) {
     ...
     if (listMatch) {
         ...
         const expectedIndent = stack.length > 0 ? stack[stack.length - 1].spaces + 2 : 0;
         let finalLine = line;
     ...
     if (isNewNode) {
         while (stack.length > 0 && stack[stack.length - 1].spaces >= structuralSpaces) {
             stack.pop();
         }
