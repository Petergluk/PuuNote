        ...
        <div className="relative flex items-center gap-2">
            <div className="flex-1 relative">
                <input 
                    ...
                    className="w-full pl-8 pr-3 py-2 rounded-lg border border-app-border bg-app-input-bg text-app-text-primary focus:outline-app-accent text-sm"
                />
...
export function SettingsComponent() {
    ...
    return <div className="flex flex-col gap-4 text-app-text-primary">
            ...
            <div className="flex-1 overflow-auto space-y-6">
                {prompts.map(prompt => (
                    <div key={prompt.id} className={`pb-6 border-b border-app-border flex flex-col gap-3 relative transition-opacity ${prompt.isActive === false ? 'opacity-50 grayscale' : ''} last:border-0 last:pb-0`}>
                        <div className="absolute top-0 right-0 flex items-center justify-end gap-3 z-10 pl-2 pb-2">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 sm:pr-32">
                            <label className="flex-1">
                                <div className="text-xs font-semibold text-app-text-muted mb-1">Название (показать в тултипе)</div>
                                <input value={prompt.label} onChange={e => handleUpdate(prompt.id, { label: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-input-bg text-app-text-primary focus:border-app-accent focus:ring-1 focus:ring-inset focus:ring-app-accent outline-none transition-shadow" />
                            </label>
                            <div className="w-full sm:w-64 z-10 flex-shrink-0">
                        ...
                        <label>
                            ...
                            <textarea value={prompt.template} onChange={e => handleUpdate(prompt.id, { template: e.target.value })} rows={5} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-input-bg text-app-text-primary focus:border-app-accent focus:ring-1 focus:ring-inset focus:ring-app-accent outline-none font-mono text-sm resize-y min-h-[120px] transition-shadow" />
                        </label>
