...
const DEFAULT_CHAINS: TaskChain[] = [
  {
    id: "humor-v2",
    name: "Юмористический рерайт (Новый)",
    enabled: true,
    ...
    steps: [
      {
        id: "h2-1",
        instruction: `Для каждого абзаца нижеследующего текста:

ШАГ 1. АНАЛИЗ
- ПОДШАГ 1. НАЙДИ ПРОТИВОРЕЧИЕ (Конфликт, угроза благополучию/идентичности/норме, несовместимые системы отсчета).
- ПОДШАГ 2. УСИЛЕНИЕ (Яркие детали, раскачка напряжения).
- ПОДШАГ 3. ПАНЧ (Выворот противоречия в неожиданную сторону).
Проанализируй то, что получилось и выбери направление: Вверх, Вниз, Влево, Вправо, В лоб, Назад или Внутрь.

ШАГ 2. ПЕРЕПИШИ АБЗАЦ - 3 ВАРИАНТА 
Перепиши абзац 3 разными способами, сохраняя смысл или увеличивая обьем за счет неожиданных метафор. Используй стили юмористических писателей (Льюис Кэролл, Ильф и Петров, Пелевин, О. Генри и тд).

ШАГ 3. ОЦЕНКА
Перечитай 3 варианта и отметь, где твой внутренний читатель улыбнулся или заржал.

Выведи это для каждого абзаца исходного текста.
Исходный текст:
{{step_0}}`,
      },
      {
        id: "h2-2",
        instruction: `Теперь собери самые удачные рерайты из предыдущего шага в новый, цельный финальный текст. 
Комбинируй наиболее удачные шутки из разных вариантов. Сделай текст стилистически единым. 
Выведи ТОЛЬКО финальный текст.

Наработки из предыдущего шага:
{{step_1}}`,
      },
      {
        id: "h2-3",
        systemPrompt: "Ты строгий JSON-форматтер. Возвращай только валидный JSON-массив, без маркдаун-оболочек.",
        instruction: `Теперь собери все наработки в строго древовидную структуру JSON (массив объектов). 
Требование пользователя: "К каждому исходному блоку текста привязан блок переписанного текста. А третьим уровнем к этому блоку привязаны оставшиеся варианты и разборы".

Формат (строго массив объектов, без \`\`\`json):
[
  {
    "content": "Исходный абзац 1",
    "children": [
      {
        "content": "Лучший вариант рерайта для первого абзаца",
        "children": [
          { "content": "Вариант 2: ..." },
          { "content": "Вариант 3: ..." },
          { "content": "Разбор (Противоречие, Усиление, Панч, Оценка): ..." }
        ]
      }
    ]
  },
  {
    "content": "Исходный абзац 2",
    "children": [
      {
        "content": "Лучший вариант рерайта для второго абзаца",
        "children": [
          { "content": "Вариант 2: ..." },
          { "content": "Вариант 3: ..." },
          { "content": "Разбор: ..." }
...
Анализ и 3 варианта по каждому абзацу:
{{step_1}}
