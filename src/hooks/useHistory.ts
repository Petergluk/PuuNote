...
export function useHistory<T>(initialValue: T | (() => T), maxHistory = 50) {
  const [history, setHistory] = useState<{ past: T[], present: T, future: T[] }>(() => {
    const present = typeof initialValue === 'function' ? (initialValue as Function)() : initialValue;
    return {
      past: [],
      present,
      future: []
  ...
  const set = useCallback((value: T | ((prev: T) => T)) => {
    setHistory((currentState) => {
      const nextPresent = typeof value === 'function' ? (value as Function)(currentState.present) : value;
